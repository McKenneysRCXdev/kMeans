...
def demonstrationData(trainingFileName, testingFileName, trainingFileHeaderNum = 0, testingFileHeaderNum = 0):
    ...
    anomArrTrain, anomPercentTrain = testCSVdata(trainingFileName, trainingFileHeaderNum, BestMeanSet, BestThresholdSet, columnInfo)
    print("{0}% of the training data contains anomalies.".format(round(anomPercentTrain * 100, 0)))

    anomArrTest, anomPercentTest = testCSVdata(testingFileName, testingFileHeaderNum, BestMeanSet, BestThresholdSet, columnInfo)
...
def testCSVdata(csvFilename, testingFileHeaderNum, BestMeanSet,BestThresholdSet, columnInfo):
    ...
    for i in range(len(BestMeanSet)):
        unNormalizedMean = columnInfo.unNormalizedMean(BestMeanSet[i])
        unNormalizedMean.append("Mean")
        thewriter.writerow(unNormalizedMean)
    csvfile.close()
    csvFlagFile.close()
    return anomArr, anomPercent

def trainingFromCSVdata(CSVfilename, trainingFileHeaderNum=0, percentPointsToCapture = 0.9):
...
def optimumMeans(dataSet, columnSet, numIterationsPerMean = 3, percentPointsToCapture = 0.9):
    ...
    bestMeanSet = None
    bestNumberOfMeans = 0
    NewMeanPenalty = 0.25 * (numberOfTrainingPoints * columnSet.discretePenalty) / numberOfMeansTest
...
def recomputeMeans(currClassification, dataSetInUse,numOfMeans,columnSet,lastMeanSet):
    ...
    for j in range(numOfMeans):
        ...
        if len(pointsBelongingToMean) == 0:
            ...
            print("Problem with " + str(numOfMeans))
        else:
